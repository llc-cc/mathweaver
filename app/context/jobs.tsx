...
export function JobsProvider({ children }: { children: React.ReactNode }) {
  ...
  const poll = useCallback((id: string, identity: AuthRequestIdentity) => {
    const check = async () => {
      ...
      try {
        ...
        let res: Response;
        try {
          res = await protectedFetch(
            apiUrl(`/api/v2/jobs/${id}/status`),
            { signal: ctrl.signal },
          ...
          if (controllers.current[id] === ctrl) delete controllers.current[id];
        }
        if (!isAuthRequestIdentityCurrent(identity)) return;
        ...
        if (status.status === "done") {
          try {
            const rRes = await protectedFetch(
              apiUrl(`/api/v2/jobs/${id}/result`),
              {},
              identity.token ?? undefined,
            );
            if (!isAuthRequestIdentityCurrent(identity)) return;
            if (!rRes.ok) throw new Error(`result request failed: ${rRes.status}`);
  ...
  const startJob = useCallback(
    (jobId: string, filename: string, sourceMarkdown: string) => {
      ...
      poll(jobId, captureAuthRequestIdentity());
    },
    [poll]
  );

  const pauseJob = useCallback(async (jobId: string) => {
    const identity = captureAuthRequestIdentity();
    patchJob(jobId, { pendingAction: "pause" });
    try {
      const response = await protectedFetch(apiUrl(`/api/v2/jobs/${jobId}/pause`), {
        method: "POST",
      }, identity.token ?? undefined);
      if (!isAuthRequestIdentityCurrent(identity)) return false;
  ...
  const resumeJob = useCallback(async (jobId: string) => {
    ...
    try {
      const response = await protectedFetch(apiUrl(`/api/v2/jobs/${jobId}/resume`), {
        method: "POST",
      }, identity.token ?? undefined);
      if (!isAuthRequestIdentityCurrent(identity)) return false;
      const body = await response.json().catch(() => ({}));
  ...
  const cancelJob = useCallback(async (jobId: string) => {
    ...
    try {
      const response = await protectedFetch(apiUrl(`/api/v2/jobs/${jobId}/cancel`), {
        method: "POST",
      }, identity.token ?? undefined);
      if (!isAuthRequestIdentityCurrent(identity)) return false;
      const body = await response.json().catch(() => ({}));
  ...
  const restoreJob = useCallback((job: RestoredJob) => {
    ...
    poll(job.job_id, captureAuthRequestIdentity());
  }, [poll]);
