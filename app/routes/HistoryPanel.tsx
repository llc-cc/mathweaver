...
export function HistoryPanel({ token, llmConfig, onLoad, onResume, onClose }: HistoryPanelProps) {
  ...
  useEffect(() => {
    setLoading(true);
    setError("");
    fetchWithTimeout(apiUrl("/api/v2/history"), { headers: authHeader(token) })
      .then(async (r) => {
  ...
  const openItem = async (id: string, filename: string) => {
    ...
    setError("");
    try {
      const res = await fetchWithTimeout(apiUrl(`/api/v2/history/${id}`), { headers: authHeader(token) });
  ...
  const deleteItem = async (id: string) => {
    ...
    setError("");
    try {
      const res = await fetchWithTimeout(apiUrl(`/api/v2/history/${id}`), { method: "DELETE", headers: authHeader(token) });
