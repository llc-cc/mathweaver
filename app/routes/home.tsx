...
import { MathText, SmartTitle, plainTextFromMathText } from "./math";
...
import {
  EducationHub,
  assignmentGraphResult,
  loadEducationAssignment,
  loadEducationSnapshot,
  snapshotGraphResult,
...
import { nodeStatementText } from "./source-matching";
import {
  MAIN_PIPELINE_STAGE_DEFS as STAGE_DEFS,
  pipelineStageLabel,
} from "../pipeline-stages";
import {
  loadAuth, saveAuth, clearAuth, loadNodeLanguage, saveNodeLanguage,
...
export function labelText(raw: string): string {
  return plainTextFromMathText(raw);
}
...
const EDUCATION_WORKSPACE_MODES: Array<{
  ...
  { mode: "course", label: "课程学习", compactLabel: "课程", title: "查看课程知识图谱并进入学习空间", Icon: GraduationCap },
  { mode: "create", label: "自主建图", compactLabel: "建图", title: "上传资料制作我的知识图谱", Icon: GitBranch },
];
...
interface UploadScreenProps {
  onSubmit: (content: string, filename: string, llm: LLMConfig, sourceOrigin?: "markdown" | "ocr") => Promise<void>;
  llm: LLMConfig;
...
function UploadScreen({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let sourceOrigin: "markdown" | "ocr" = "markdown";
      ...
      await onSubmit(content, filename, llm, sourceOrigin);
    } catch (e) {
    ...
    <div className="mg-root">
      <div className="mg-upload-screen">
        ...
        <div className="mg-upload-auth-bar">
          {auth ? (
            <>
              <span className="mg-auth-email" style={{ fontSize: 11, color: "var(--muted)", background: "var(--surface-alt)", border: "1px solid var(--line)", borderRadius: 999, padding: "3px 10px" }}>{auth.email}</span>
  ...
  return (
    <div className="mg-drawer open">
      <div className="mg-drawer-inner" style={{ position: "relative" }}>
        ...
        {/* Content */}
        {nodeStatementText(node) && (
          <div className="mg-detail-section">
            <div className="mg-detail-section-label">原文</div>
            <MatrixFlowText text={nodeStatementText(node)} flows={node.matrix_flows} field="statement" audience="author" macros={macros} className="mg-detail-text" layoutMode="vertical" />
          </div>
        )}
        ...
        <ProofWorkspace graphId={graphId} node={node} token={token} llmConfig={llmConfig} macros={macros} matrixFlowAudience="author" matrixFlowLayoutMode="vertical" />
...
function ResultScreen({ workspaceMode, result, filename, jobId, sourceMarkdown, onReset, auth, llmConfig, onShowHistory, onShowAuth, onShowSettings, onShowApiGuide, onShowStudio, onSourceMarkdownRecover, nodeLanguage, onNodeLanguageChange }: ResultScreenProps) {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl(`/api/v2/history/${jobId}/markdown`), { headers: { Authorization: `Bearer ${auth.token}` } })
  ...
  useEffect(() => {
    ...
    const autoSave = async () => {
      setSaving(true);
      try {
        const res = await fetch(apiUrl("/api/v2/history"), {
          method: "POST",
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const res = await fetch(apiUrl(`/api/v2/export/${jobId}`), { method: "POST" });
...
function ErrorScreen({
  ...
  const loadErrorDetail = useCallback(async (force = false) => {
    ...
    try {
      ...
      const response = await fetch(apiUrl(`/api/v2/jobs/${encodeURIComponent(jobId)}/error-detail`), {
...
function SettingsModal({
  profiles, activeIndex, auth, onSave, onClose, onLogout, nodeLanguage,
...
export default function Home() {
  ...
  // Assignment links are course-learning links even when an older caller did
  // not include the explicit `edu=hub` flag. Keep the mode derived from the
  ...
  const [educationTarget, setEducationTarget] = useState<GraphNode | null>(null);
  const [educationTargetCourseGraphId, setEducationTargetCourseGraphId] = useState<string | null>(null);
  const [educationTargetClassId, setEducationTargetClassId] = useState<string | null>(null);
  ...
  const [educationResumeTarget, setEducationResumeTarget] = useState<{ kind: "assignment" | "courseGraph"; id: string } | null>(null);
  const [courseImportOpen, setCourseImportOpen] = useState(false);
  const [courseImportTarget, setCourseImportTarget] = useState<GraphNode | null>(null);
  const [courseImportClasses, setCourseImportClasses] = useState<EducationClass[]>([]);
  const [courseImportClassId, setCourseImportClassId] = useState("");
  ...
  const setEducationCourseGraphLocation = useCallback((snapshotId?: string | null) => {
    setSearchParams((current) => {
      const next = new URLSearchParams(current);
      next.set("edu", "hub");
      next.delete("assignment");
  ...
  const closeEducation = useCallback(() => {
    ...
    setEducationTargetClassId(null);
    setEducationError("");
  ...
  const returnToEducationHub = useCallback((classId?: string | null) => {
    setEducationAssignment(null);
    setEducationCourseGraph(null);
    setEducationTarget(null);
    setEducationTargetCourseGraphId(null);
    setEducationTargetClassId(classId || null);
    setEducationError("");
    setEducationProfileClassId(null);
    setEducationResumeTarget(null);
    setEducationDraftDirty(false);
  ...
  const openEducationCourseGraph = useCallback((snapshotId: string) => {
    ...
    setEducationProfileClassId(null);
    setEducationResumeTarget(null);
    setEducationCourseGraphLocation(snapshotId);
    if (!auth || !auth.educationRole) setShowAuthModal(true);
  }, [auth, setEducationCourseGraphLocation]);
  const openEducationTargetFromCourseGraph = useCallback((snapshotId: string, classId: string, node: GraphNode) => {
    setEducationTarget(node);
    setEducationTargetCourseGraphId(snapshotId);
    setEducationTargetClassId(classId);
    setEducationAssignment(null);
    setEducationCourseGraph(null);
    setEducationError("");
    setEducationProfileClassId(null);
  ...
  const openCourseImport = useCallback((target?: GraphNode | null) => {
    if (!auth || auth.educationRole !== "teacher") return;
    setCourseImportTarget(target || null);
    setCourseImportClasses([]);
    setCourseImportClassId("");
    setCourseImportError("");
    setCourseImportLoading(true);
    setCourseImportOpen(true);
    fetch(apiUrl("/api/v2/edu/classes"), { headers: authHeaders(auth.token) })
      .then(async response => {
        const body = await response.json().catch(() => ({})) as { classes?: EducationClass[]; error?: string };
        if (!response.ok) throw new Error(body.error || "无法加载班级列表");
        const teacherClasses = (body.classes || []).filter(item => item.role === "teacher");
        setCourseImportClasses(teacherClasses);
        setCourseImportClassId(teacherClasses.length === 1 ? teacherClasses[0].id : "");
      })
      .catch(cause => setCourseImportError(cause instanceof Error ? cause.message : String(cause)))
      .finally(() => setCourseImportLoading(false));
  }, [auth]);
  const confirmCourseImport = useCallback(async () => {
    if (!auth || auth.educationRole !== "teacher" || !result || !jobId || !courseImportClassId) return;
    setCourseImportBusy(true);
    setCourseImportError("");
    try {
      const response = await fetch(apiUrl(`/api/v2/edu/classes/${encodeURIComponent(courseImportClassId)}/snapshots`), {
        method: "POST",
        headers: authHeaders(auth.token),
        body: JSON.stringify({
          sourceGraphId: jobId,
          sourceJobId: jobId,
          filename: filename || "教学图谱",
          nodes: result.nodes,
          edges: result.edges,
          sourceMarkdown: sourceMarkdown || "",
          latexMacros: result.latex_macros || {},
        }),
      });
      const body = await response.json().catch(() => ({})) as { snapshot?: EducationSnapshot; created?: boolean; error?: string };
      if (!response.ok || !body.snapshot) throw new Error(body.error || "导入课程失败");
      const target = courseImportTarget;
      setCourseImportOpen(false);
      setEducationTarget(target);
      setEducationTargetCourseGraphId(body.snapshot.id);
      setEducationTargetClassId(courseImportClassId);
      setEducationAssignment(null);
      setEducationCourseGraph(null);
      setEducationError("");
      setEducationProfileClassId(null);
      setEducationResumeTarget(null);
      setEducationEntryNotice(body.created === false ? "该课程图谱已存在，已进入所选班级" : "当前图谱已导入课程");
      setEducationLocation(null);
    } catch (cause) {
      setCourseImportError(cause instanceof Error ? cause.message : String(cause));
    } finally {
      setCourseImportBusy(false);
    }
  }, [auth, courseImportClassId, courseImportTarget, filename, jobId, result, setEducationLocation, sourceMarkdown]);
  ...
  const fetchAccountSettings = async (token: string) => {
    try {
      const res = await fetch(apiUrl("/api/v2/settings"), { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) return;
  ...
  useEffect(() => {
    ...
    const hydrateAuth = async () => {
      try {
        const res = await fetch(apiUrl("/api/v2/auth/me"), { headers: authHeaders(a.token) });
        if (res.status === 401) {
          clearAuth();
          setAuth(null);
          return;
        ...
        if (res.ok) {
          const data = await res.json() as { email?: string; educationRole?: string | null; canTeach?: boolean };
          const educationRole: EducationRole | null = data.educationRole === "teacher"
            ? "teacher"
            : data.educationRole === "student" ? "student" : null;
          const synced = { token: a.token, email: data.email || a.email, educationRole, canTeach: Boolean(data.canTeach) };
          saveAuth(synced.token, synced.email, synced.educationRole, synced.canTeach);
          setAuth(synced);
        }
        await fetchAccountSettings(a.token);
      } catch {
        await fetchAccountSettings(a.token);
      } finally {
        setConfigReady(true);
  ...
  useEffect(() => {
    ...
    setEducationError("");
    const load = requestedAssignmentId
      ? loadEducationAssignment(auth.token, requestedAssignmentId).then(assignment => {
          if (!cancelled) { setEducationAssignment(assignment); setEducationCourseGraph(null); }
        })
      : loadEducationSnapshot(auth.token, requestedCourseGraphId!).then(snapshot => {
          if (!cancelled) { setEducationCourseGraph(snapshot); setEducationAssignment(null); }
        });
    load
      .catch(cause => {
        if (cancelled) return;
        const error = cause as { code?: string; classId?: string; message?: string };
        if (error.code === "student_profile_required") {
          setEducationProfileClassId(error.classId || null);
          setEducationResumeTarget(requestedAssignmentId
            ? { kind: "assignment", id: requestedAssignmentId }
            : { kind: "courseGraph", id: requestedCourseGraphId! });
          setEducationLocation(null);
          return;
        }
        setEducationError(cause instanceof Error ? cause.message : String(cause));
      })
  ...
  useEffect(() => {
    ...
    void fetch(apiUrl("/api/v2/history"), {
      method: "POST",
      headers: authHeaders(auth.token),
      body: JSON.stringify({ job_id: jobId }),
    }).catch(() => {
  ...
  const handleSubmit = async (content: string, fname: string, cfg: LLMConfig, sourceOrigin: "markdown" | "ocr" = "markdown") => {
    ...
    try {
      const res = await fetch(apiUrl("/api/v2/jobs"), {
        method: "POST",
  ...
  const handleAuth = (token: string, email: string, educationRole?: "teacher" | "student" | null, canTeach?: boolean) => {
    saveAuth(token, email, educationRole, canTeach);
    setAuth({ token, email, educationRole, canTeach });
    setShowAuthModal(false);
  ...
  const handleGuideComplete = async (config: LLMConfig, profileName: string) => {
    if (auth) {
      ...
      const response = await fetch(apiUrl("/api/v2/settings"), {
        method: "PUT",
        headers: authHeaders(auth.token),
  ...
  return (
    <>
      ...
      {courseImportOpen && (
        <div className="edu-modal-backdrop edu-course-import-layer" data-theme={resolveTheme(loadStudioSettings().theme)} onClick={() => { if (!courseImportBusy) setCourseImportOpen(false); }}>
          <section className="edu-settings-modal edu-course-import-class-modal" role="dialog" aria-modal="true" aria-labelledby="edu-course-import-class-title" onClick={event => event.stopPropagation()}>
            <div className="edu-settings-head">
              <div><span className="edu-kicker">课程图谱</span><h2 id="edu-course-import-class-title">导入课程</h2></div>
              <button className="edu-icon-button" disabled={courseImportBusy} onClick={() => setCourseImportOpen(false)} aria-label="关闭"><X size={17} /></button>
            </div>
            <p className="edu-settings-copy">
              将“{filename || "当前图谱"}”加入所选班级。{courseImportTarget ? `随后将节点“${courseImportTarget.title_zh || courseImportTarget.title_en || `节点 ${courseImportTarget.id}`}”预选为学习目标。` : "加入后班级成员即可查看。"}
            </p>
            {courseImportLoading ? (
              <div className="edu-course-import-loading"><Loader2 className="edu-spin" size={16} />正在加载班级…</div>
            ) : courseImportClasses.length ? (
              <div className="edu-course-import-class-list" role="radiogroup" aria-label="选择班级">
                {courseImportClasses.map(item => (
                  <label className={`edu-course-import-class ${courseImportClassId === item.id ? "selected" : ""}`} key={item.id}>
                    <input type="radio" name="course-import-class" value={item.id} checked={courseImportClassId === item.id} disabled={courseImportBusy} onChange={() => setCourseImportClassId(item.id)} />
                    <span><strong>{item.title}</strong><small>{item.memberCount ?? 0} 位成员 · {item.assignmentCount ?? 0} 个任务</small></span>
                  </label>
                ))}
              </div>
            ) : !courseImportError ? (
              <div className="edu-course-import-empty">
                <span>还没有可用班级，请先进入课程学习创建班级。</span>
                <button className="edu-button secondary" onClick={() => { setCourseImportOpen(false); openEducation(); }}><GraduationCap size={14} />进入课程学习</button>
              </div>
            ) : null}
            {courseImportError && <div className="edu-modal-error">{courseImportError}</div>}
            {courseImportClasses.length > 0 && (
              <div className="edu-row-actions edu-confirm-actions">
                <button className="edu-button ghost" disabled={courseImportBusy} onClick={() => setCourseImportOpen(false)}>取消</button>
                <button className="edu-button primary" disabled={!courseImportClassId || courseImportBusy} onClick={() => void confirmCourseImport()}>
                  {courseImportBusy ? <><Loader2 className="edu-spin" size={15} />正在导入…</> : <><GraduationCap size={15} />确认导入</>}
                </button>
              </div>
            )}
          </section>
      ...
      <FloatingBadge
        hidden={educationRequested || (view === "result" && experience === "studio" && studioNodeSelected)}
        onViewResult={(jid) => {
          const job = jobs[jid];
          if (job?.result) {
            setStudioNodeSelected(false);
      ...
      <div style={{ display: !educationRequested && workspaceMode === "generate" && view === "upload" ? "contents" : "none" }}>
        <UploadScreen
      ...
      {!educationRequested && view === "result" && result && experience === "studio" && (
        <StudioWrapper
          workspaceMode={workspaceMode}
          result={result}
          filename={filename}
      ...
      {!educationRequested && view === "error" && errorInfo && (
        <ErrorScreen
      ...
      {educationRequested && auth && auth.educationRole && !requestedAssignmentId && !requestedCourseGraphId && !requestedClassStatisticsId && (
        <EducationHub
          token={auth.token}
          educationRole={auth.educationRole}
          targetNode={educationTarget}
          initialCourseGraphId={educationTargetCourseGraphId}
          ...
          initialNotice={educationEntryNotice}
      ...
      {educationRequested && auth && auth.educationRole && educationAssignment && !educationLoading && (() => {
        ...
        return (
          <StudioWrapper
            ...
            result={educationGraph}
            filename={educationAssignment.snapshot.filename}
            jobId={educationAssignment.snapshot.id}
            sourceMarkdown={educationAssignment.snapshot.sourceMarkdown}
            nodeLanguage={nodeLanguage}
            token={auth.token}
            llmConfig={llm}
            onNodeSelectionChange={setStudioNodeSelected}
            onReset={() => returnToEducationHub(educationAssignment.classId)}
            learningAssignment={educationAssignment}
            onOpenEducation={() => returnToEducationHub(educationAssignment.classId)}
      ...
      {educationRequested && auth && auth.educationRole && requestedCourseGraphId && !requestedAssignmentId && educationCourseGraph && !educationLoading && (
        <StudioWrapper
          workspaceMode="import"
          result={snapshotGraphResult(educationCourseGraph)}
          filename={educationCourseGraph.filename}
          jobId={educationCourseGraph.id}
          sourceMarkdown={educationCourseGraph.sourceMarkdown}
          nodeLanguage={nodeLanguage}
          token={auth.token}
          llmConfig={llm}
          onNodeSelectionChange={setStudioNodeSelected}
          onReset={() => returnToEducationHub(educationCourseGraph.classId)}
