...
export const EDGE_KINDS: Record<EdgeKind, EdgeKindMeta> = {
  ...
  defines:     { key: "defines",     label: "定义依赖",     color: "#8a8f98", colorDark: "#5f636a" },
...
/** Relationship kinds currently exposed by GraphStudio. */
export const ACTIVE_EDGE_KINDS: readonly EdgeKind[] = ["derives", "defines"];

const DEFINITION_EDGE_LABEL = /^(?:定义依赖|定义引用|引用定义|definition[_ -]?(?:dependency|reference)|definitional dependency|defines?)$/i;
const DERIVATION_EDGE_LABEL = /^(?:逻辑依赖|逻辑推导|推导|依赖|前置|logical dependency|derivation|derives?|dependency|prerequisite)$/i;

/** Classify an edge for the current UI using only its backend relation label. */
export function classifyVisibleEdge(e: GraphEdge): EdgeKind {
  const label = String(e.label || "").trim();
  if (DEFINITION_EDGE_LABEL.test(label)) return "defines";
  if (DERIVATION_EDGE_LABEL.test(label)) return "derives";
  // Keep legacy/imported edges visible while collapsing unavailable kinds into
  // the currently supported dependency presentation.
  return "derives";
}
