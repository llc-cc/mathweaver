...
curl --fail --silent --show-error "$BASE_URL/api/v2/ping" >"$TMP_DIR/ping.json"
python3 -c 'import json,sys; data=json.load(open(sys.argv[1], encoding="utf-8")); assert data == {"ok": True}' "$TMP_DIR/ping.json"
...
curl --fail --silent --show-error -X POST -H "Authorization: Bearer ${TOKEN}" "$BASE_URL/api/v2/auth/logout" >/dev/null
