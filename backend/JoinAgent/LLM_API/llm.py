...
def _resolve_requests_proxy():
    explicit_proxy = os.getenv("PDFPIPELINE_LLM_PROXY")
    if explicit_proxy is None:
        explicit_proxy = os.getenv("LLM_HTTP_PROXY")
    if explicit_proxy is None:
        explicit_proxy = os.getenv("PDFPIPELINE_PROXY")
    explicit_proxy = (explicit_proxy or "").strip()
    if explicit_proxy.lower() in {"direct", "none", "off", "false", "0"}:
        return None
    if explicit_proxy:
        proxy = explicit_proxy if "://" in explicit_proxy else "http://" + explicit_proxy
...
class SimpleLLM:
    ...
    def __init__(self, model='gemini-3-flash-preview', api_url=None, api_key=None):
        """
        初始化 SimpleLLM
        
        Args:
            model: 模型名称
            api_url: API 地址（可选，默认使用硬编码的地址）
            api_key: API 密钥（可选，默认使用硬编码的密钥）
        """
        # ✅ 如果用户提供了 API 配置，使用用户的；否则从环境变量读取
        if api_key:
            self.api_key = api_key
        else:
            self.api_key = os.getenv('PDFPIPELINE_API_KEY', '')
        if api_url:
            self.api_url = _normalize_chat_completions_url(api_url)
        else:
            env_url = os.getenv('PDFPIPELINE_API_URL', 'https://aihubmix.com/v1')
            self.api_url = _normalize_chat_completions_url(env_url)
        self.connect_timeout = _env_positive_float("LLM_HTTP_CONNECT_TIMEOUT", 15.0)
        self.read_timeout = _env_positive_float("LLM_HTTP_TIMEOUT", 300.0)
        self.request_timeout = (self.connect_timeout, self.read_timeout)
        self.model = model
        self.session = requests.Session()
        self.session.trust_env = False
        proxy = _resolve_requests_proxy()
        if proxy:
            self.session.proxies.update(proxy)
        self.proxy = proxy
        self.suppress_error_details = False
    ...
    def ask(self, prompt, temperature=1):
        ...
        if os.getenv("LLM_DEBUG"):
            proxy_text = self.proxy if self.proxy else "direct"
            print(
                f"LLM request start model={self.model} prompt_chars={len(prompt)} "
                f"timeout=({self.connect_timeout:g}s connect, {self.read_timeout:g}s read) "
                f"proxy={proxy_text}",
                flush=True,
            )
        try:
            ...
            if self.suppress_error_details:
                print(
                    f"LLM HTTP failed model={self.model} status={status} "
                    f"elapsed={time.time()-start:.1f}s {type(e).__name__}"
                )
            else:
                body = getattr(getattr(e, "response", None), "text", "") or ""
                print(f"LLM HTTP failed model={self.model} status={status} "
                      f"elapsed={time.time()-start:.1f}s {type(e).__name__}: {str(e)[:120]} body={body[:160]!r}")
            raise
        response_json = response.json()
        if os.getenv("LLM_DEBUG"):
            print(f"LLM ok model={self.model} elapsed={time.time()-start:.1f}s")

        if 'choices' in response_json and len(response_json['choices']) > 0:
            return response_json['choices'][0]['message']['content']
        else:
            print(f"LLM response has no choices model={self.model} resp={str(response_json)[:200]}")
