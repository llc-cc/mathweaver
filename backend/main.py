...
from ocr_runtime import OcrError, get_ocr_manager
...
def run_pipeline(
    ...
    source_origin = source_origin or ("ocr" if file_ext == ".pdf" else "markdown")
    ...
    if file_ext in {'.md', '.tex'}:
        ...
        if os.path.exists(legacy_md_path):
            ...
            # 若文件不存在，复用桌面 API 的本地 OCR 运行时和 Markdown 契约
            os.makedirs(final_output_dir, exist_ok=True) # 确保目录在 MinerU 运行前就存在
            print(f"▶️  正在启动 MinerU 进行转换...")
            ocr_result = get_ocr_manager().parse_local_file(input_path)
            Path(final_md_path).write_text(ocr_result["importedText"], encoding="utf-8")
            md_file_path = final_md_path
            print("✅ MinerU 转换成功。")

        # ---------------------------------------------------------
        # 步骤 2: 整理 Markdown 归档路径 (兼容性处理)
        # ---------------------------------------------------------
        print("\n>>> [Step 2/3] 检查文件结构清洗状态...")
        ...
        if skipped_ocr:
            ...
            print("✅ Markdown 已按桌面 OCR 结果契约写入目标目录。")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"❌ MinerU 转换失败 [{exc.code}]: {exc.message}")
