...
def _sync_teacher_accounts(conn) -> None:
    accounts = _configured_teacher_accounts()
    conn.execute("UPDATE users SET can_teach = 0")
    now = datetime.utcnow().isoformat()
    for account in accounts:
        ...
        if existing:
            ...
            conn.execute(
                "INSERT INTO users (email, password_hash, can_teach, created_at) VALUES (?, ?, 1, ?)",
                (account["email"], password_hash, now),
            )
