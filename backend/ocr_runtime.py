...
class OcrManager:
    ...
    def get_result(self, job_id: str) -> dict[str, Any]:
        ...
        return {"importedText": path.read_text(encoding="utf-8", errors="replace"), "filename": job["filename"], "source": "ocr_file", "ocr_job_id": job_id}
    ...
    def _run_job(self, job_id: str, cancel_event: threading.Event) -> None:
        ...
        try:
            ...
            markdown = self.engine.parse(Path(upload["source_path"]), cancel_event)
            if cancel_event.is_set():
                raise OcrCancelled()
